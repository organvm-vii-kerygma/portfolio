...
describe('HTML build-link extraction', () => {
	it('ignores JavaScript template links but retains script source attributes', () => {
		const html = '<script src="engine.js">const a = `<a href="${esc(docs[ref])}">Docs</a>`;</script><a href="real.html">Real</a>';
	...
	it('ignores comments, CSS, textareas, titles and escaped markup', () => {
		const html = '<title>&lt;a href="title"&gt;</title><!-- <a href="comment"> --><style>.x{content:\'<a href="css">\'}</style><textarea><a href="text">Text</a></textarea><pre>&lt;a href="escaped"&gt;</pre>';
		expect(collectHtmlLinks(html)).toEqual([]);
	});

	it('supports valid attribute spacing, case, quotes, unquoted values and entities', () => {
		const html = '<A HREF = "page.html?a=1&amp;b=2">A</A><img src=photo.png><a href=\'other.html\'>B</a>';
	...
	it('retains actual missing-file links for downstream validation', () => {
		const html = '<script>const fake = \'<a href="ignored.html">\';</script><a href="missing.html">Broken</a><img src="missing.png">';
	...
	it('retains stylesheets and excludes data attributes', () => {
		expect(collectHtmlLinks('<link rel="stylesheet" href="site.css"><div data-href="not-a-link"></div>')).toEqual(['site.css']);
	});
